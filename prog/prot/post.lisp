...
#+a(DEFun Po-CONSTRUCT.END ()
  ...
  (when po*file
	...
	(when (consp ops)
	  (mapc #'(lambda (str) (princ str po*file))
		(nreverse ops))))
      (when (EQL (CAR RESULT) 'SUCCESS) (po=prover po*file))))) ;;; deleted 23.1.95 Kerber, Huang, Sehn
...
(DEFun Po-CONSTRUCT.END ()
  ...
  (when po*file
	...
	(when (consp ops)
	  (mapc #'(lambda (str) (princ str po*file))
		(nreverse ops))))
      ;(po=prover po*file) ;;; deleted 23.1.95 Kerber, Huang, Sehn
      )))
...
;(PO=REPL.OP.ONE.RES #<EXCL::STRING-OUTPUT-STREAM @ #xa556b2> 9 ((9 . 1) (8 . 1) NIL) NIL NIL 1 NIL "A2" NIL NIL)
...
(defun po=repl.op.implicit.multiple (file resolvent unifier act.lits assocs)
  (let (cand dlit (act.litno1 0) act.litno2)
	...
	(let ()
      ...
      act.lits)))
;(PO-OPERATION REPLACEMENT.OPERATION (9 (NIL (9 8) (((9 . 1) (8 . 1) NIL)) (NIL))))
