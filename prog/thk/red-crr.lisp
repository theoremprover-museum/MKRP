			       ...
			       (COND (GEN	; generalizing replacement resolution is done and
				      ...
				      (COND ((SETQ INFO (RED=CRR_PROTOCOL.RECONSTRUCT OBJECT.CLAUSE OBJECT.LITNO OMARKS
					     ...
					     (case (opt-get.option pr_protocol)
					       (standard (PR-OPERATION 'REPLACEMENT.OPERATION OBJECT.CLAUSE INFO))
					       (post (PR-OPERATION 'REPLACEMENT.OPERATION r.link subject.clause object.clause)))
...
(DEFUN RED=CRR_FALSE.REMOVE (CLAUSE LITNO RESOLVE.CLAUSE)
  ...
  (PR-OPERATION 'REPLACEMENT.OPERATION CLAUSE
		(LIST NIL (LIST CLAUSE RESOLVE.CLAUSE)
...
(DEFUN RED=CRR_PROTOCOL.RECONSTRUCT
  ...
  (PROG
    ((OL.MARK (RDS-MARK OBJECT.MARKS OBJECT.LITNO)) RESOLUTIONS MULTIPLES CLAUSES FAILURE PATH RES.CLAUSE)
	...
	(DOUBLE	;; double literal: path of rules is constructed.
	  (COND
	    ((SETQ PATH
		   (RED=CRR_PROTOCOL.DOUBLE.LITERAL.PATH SUBJECT.CLAUSE (1+ RPTN) SUBJECT.MARKS OBJECT.CLAUSE
							 OBJECT.MARKS NIL))
