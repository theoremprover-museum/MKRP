...
(DEFUN NORM-RESET ()
 ...
 ;; VALUE: UNDEFINED.
 (SETQ NORM*FUNCTION.COUNTER 0) (SETQ NORM*CONSTANT.COUNTER 0)
 (SETQ NORM*EXPRESSION NIL)
 (COND
  ((AND NORM*FILE.VERSION (INSTREAMP NORM*FILE.VERSION)) (CLOSEFILE NORM*FILE.VERSION) (SETQ NORM*FILE.VERSION NIL))))
...
(DEFUN NORM=MOVE.ALL.INSIDE (EXPRESSION)
  ...
  (let ((VARIABLE (NORM=LEFT EXPRESSION))
    ...
    (CASE (NORM=OP RIGHT)
      ...
      ((AND AND!)
       ...
       (PROG ((NEWVAR (NORM=NEW.VARIABLE VARIABLE)))
	     (NORM=PUT=LEFT RIGHT (NORM=MOVE.ALL.INSIDE (LIST 'ALL VARIABLE (NORM=LEFT RIGHT))))
	     (NORM=PUT=RIGHT RIGHT
			     (NORM=MOVE.ALL.INSIDE (LIST 'ALL NEWVAR (NORM=REPLACE.VARIABLE VARIABLE NEWVAR
											    (NORM=RIGHT RIGHT)))))
	     (RETURN RIGHT)))
      ((OR OR!)
       (PROG
	 ...
	 (RETURN
	   (COND
	     ...
	     (NOT-IN-RIGHT.LEFT ;; ALL X (P OR QX)   --->   P OR (ALL X QX)
	      (NORM=PUT=RIGHT RIGHT (NORM=MOVE.ALL.INSIDE (LIST 'ALL VARIABLE (NORM=RIGHT RIGHT)))) RIGHT)
	     (NOT-IN-RIGHT.RIGHT ;; ALL X (PX OR Q)   --->   (ALL X PX) OR Q
	      (NORM=PUT=LEFT RIGHT (NORM=MOVE.ALL.INSIDE (LIST 'ALL VARIABLE (NORM=LEFT RIGHT)))) RIGHT)
	     (T EXPRESSION)))))
      ((IMPL IMPL!)
       (PROG
	 ((NOT-IN-RIGHT.LEFT (NOT (NORM=VARIABLE.OCCURS.IN VARIABLE (NORM=LEFT RIGHT))))
	 ...
	 (RETURN
	   (COND
	     ...
	     (NOT-IN-RIGHT.RIGHT ;; ALL X (PX IMPL Q) --->  (EX X  PX) IMPL Q))
	      (NORM=PUT=LEFT RIGHT (NORM=MOVE.EX.INSIDE (LIST 'EX VARIABLE (NORM=LEFT RIGHT)))) RIGHT)
	     (T EXPRESSION)))))
      ((EQV EQV!)
       (COND
	...
	(SETQ EXPRESSION (LIST 'NOT (NORM=MOVE.EX.INSIDE (LIST 'EX VARIABLE (NORM=LEFT RIGHT))))))
	...
	(RETURN
	  (CASE (NORM=OP RIGHT)
	    ...
	    ((OR OR!)
	     ...
	     (PROG ((NEWVAR (NORM=NEW.VARIABLE VARIABLE)))
		   (NORM=PUT=LEFT RIGHT (NORM=MOVE.EX.INSIDE (LIST 'EX VARIABLE (NORM=LEFT RIGHT))))
		   (NORM=PUT=RIGHT RIGHT
				   (NORM=MOVE.EX.INSIDE (LIST 'EX NEWVAR (NORM=REPLACE.VARIABLE VARIABLE NEWVAR
												(NORM=RIGHT RIGHT)))))
		   (RETURN RIGHT)))
	    ((AND AND!)
	     (PROG
		...
		(NOT-IN-RIGHT.RIGHT (NOT (NORM=VARIABLE.OCCURS.IN VARIABLE (NORM=RIGHT RIGHT)))))
		 ...
		 (COND
		   ...
		   (NOT-IN-RIGHT.LEFT ;; EX X (P AND QX)   --->   P AND (EX X QX)
		    (NORM=PUT=RIGHT RIGHT (NORM=MOVE.EX.INSIDE (LIST 'EX VARIABLE (NORM=RIGHT RIGHT)))) RIGHT)
	    ...
	    ((IMPL IMPL!)
	     ...
	     (PROG ((NEWVAR (NORM=NEW.VARIABLE VARIABLE)))
		   (NORM=PUT=LEFT RIGHT (NORM=MOVE.ALL.INSIDE (LIST 'ALL VARIABLE (NORM=LEFT RIGHT))))
		   (NORM=PUT=RIGHT RIGHT
				   (NORM=MOVE.EX.INSIDE (LIST 'EX NEWVAR (NORM=REPLACE.VARIABLE VARIABLE NEWVAR (NORM=RIGHT RIGHT)))))
		   (RETURN RIGHT)))
	    ((EQV EQV!)
	     (COND
	       ((AND (NOT (NORM=VARIABLE.OCCURS.IN VARIABLE (NORM=LEFT RIGHT)))
	    ...
	    (NOT ;; EX  X (NOT P)     --->   NOT (ALL X P)
	      (SETQ EXPRESSION (LIST 'NOT (NORM=MOVE.ALL.INSIDE (LIST 'ALL VARIABLE (NORM=LEFT RIGHT))))))
  ...
  (let ((OP (NORM=OP EXPRESSION)))
    (CASE OP
      ...
      ((AND OR IMPL EQV)
       ...
       (when (> LEVELCOUNTER -1)
	 (NORM=PUT=OP EXPRESSION (INTERN (format nil "~A!" op))))
...
(DEFUN NORM=REPLACE.EQV (EXPRESSION SPLITTABLE NEGATION.FLAG)
  ...
  (CASE (NORM=OP EXPRESSION)
	...
	(-LEFT (NORM=REPLACE.EQV (LIST 'NOT (NORM=LEFT EXPRESSION)) SPLITTABLE NEGATION.FLAG))
	(+RIGHT (NORM=REPLACE.EQV (NORM=COPY (NORM=RIGHT EXPRESSION)) SPLITTABLE NEGATION.FLAG))
	(-RIGHT (NORM=REPLACE.EQV (LIST 'NOT (NORM=RIGHT EXPRESSION)) SPLITTABLE NEGATION.FLAG)))
       (CASE (NORM=OP EXPRESSION)
	 (EQV
	   (COND
	     (NEGATION.FLAG (NORM=PUT=OP EXPRESSION 'OR) (NORM=PUT=LEFT EXPRESSION (LIST 'AND +LEFT +RIGHT))
			    (NORM=PUT=RIGHT EXPRESSION (LIST 'AND -LEFT -RIGHT)))
	     (T (NORM=PUT=OP EXPRESSION 'AND) (NORM=PUT=LEFT EXPRESSION (LIST 'OR +LEFT -RIGHT))
                (NORM=PUT=RIGHT EXPRESSION (LIST 'OR -LEFT +RIGHT)))))
	 (EQV!
	   (COND
	     ((ANTIVALENT SPLITTABLE NEGATION.FLAG) (NORM=PUT=OP EXPRESSION 'OR!)
	      (NORM=PUT=LEFT EXPRESSION (LIST 'AND! +LEFT +RIGHT)) (NORM=PUT=RIGHT EXPRESSION (LIST 'AND! -LEFT -RIGHT)))
	     (T (NORM=PUT=OP EXPRESSION 'AND!) (NORM=PUT=LEFT EXPRESSION (LIST 'OR! +LEFT -RIGHT))
                (NORM=PUT=RIGHT EXPRESSION (LIST 'OR! -LEFT +RIGHT)))))
...
(DEFUN NORM=REPLACE.IMPL (EXPRESSION)
  ...
  (CASE (NORM=OP EXPRESSION)
    ...
    ((AND OR IMPL EQV AND! OR! IMPL! EQV!) (NORM=PUT=LEFT EXPRESSION (NORM=REPLACE.IMPL (NORM=LEFT EXPRESSION)))
     ...
     (CASE (NORM=OP EXPRESSION)
       (IMPL (NORM=PUT=OP EXPRESSION 'OR) (NORM=PUT=LEFT EXPRESSION (LIST 'NOT (NORM=LEFT EXPRESSION))))
       (IMPL! (NORM=PUT=OP EXPRESSION 'OR!) (NORM=PUT=LEFT EXPRESSION (LIST 'NOT (NORM=LEFT EXPRESSION)))) (OTHERWISE NIL))
     EXPRESSION)
    ((+ -) EXPRESSION)
    ...
    (NOT (SETQ EXPRESSION (NORM=LEFT EXPRESSION))
      (CASE (NORM=OP EXPRESSION)
        (ALL (NORM=PUT=OP EXPRESSION 'EX)
          (NORM=PUT=RIGHT EXPRESSION (NORM=MOVE.NEGATIONS.TO.LITERALS (LIST 'NOT (NORM=RIGHT EXPRESSION)))) EXPRESSION)
        (EX (NORM=PUT=OP EXPRESSION 'ALL)
          (NORM=PUT=RIGHT EXPRESSION (NORM=MOVE.NEGATIONS.TO.LITERALS (LIST 'NOT (NORM=RIGHT EXPRESSION)))) EXPRESSION)
        (AND (NORM=PUT=OP EXPRESSION 'OR)
          (NORM=PUT=LEFT EXPRESSION (NORM=MOVE.NEGATIONS.TO.LITERALS (LIST 'NOT (NORM=LEFT EXPRESSION))))
          (NORM=PUT=RIGHT EXPRESSION (NORM=MOVE.NEGATIONS.TO.LITERALS (LIST 'NOT (NORM=RIGHT EXPRESSION)))) EXPRESSION)
        (AND! (NORM=PUT=OP EXPRESSION 'OR!)
          (NORM=PUT=LEFT EXPRESSION (NORM=MOVE.NEGATIONS.TO.LITERALS (LIST 'NOT (NORM=LEFT EXPRESSION))))
          (NORM=PUT=RIGHT EXPRESSION (NORM=MOVE.NEGATIONS.TO.LITERALS (LIST 'NOT (NORM=RIGHT EXPRESSION)))) EXPRESSION)
...
(DEFUN NORM=SKOLEMIZE (EXPRESSION ENVIRONMENT)
  ...
  (CASE (NORM=OP EXPRESSION)
    ...
    (EX
      (PROG
        ((EX.VARIABLE (NORM=LEFT EXPRESSION)) SKOLEM.TERM (RIGHT (NORM=SKOLEMIZE (NORM=RIGHT EXPRESSION) ENVIRONMENT)))
        (SETQ SKOLEM.TERM (NORM=NEW.TERM ENVIRONMENT EX.VARIABLE))
        (SETQ RIGHT (NORM=REPLACE.VARIABLE EX.VARIABLE SKOLEM.TERM RIGHT)) (NORM=DROP.VARIABLE EX.VARIABLE) (RETURN RIGHT)))
...
(DEFUN NORM=MULTIPLY.TO.CNF (EXPRESSION)
  ...
  (NORM=PUT=OP EXPRESSION 'AND)
          (NORM=PUT=LEFT EXPRESSION (NORM=MULTIPLY.TO.CNF (LIST 'OR (NORM=LEFT LEFT) RIGHT)))
          (NORM=PUT=RIGHT EXPRESSION (NORM=MULTIPLY.TO.CNF (LIST 'OR (NORM=RIGHT LEFT) (NORM=COPY RIGHT)))) EXPRESSION)
        ((EQL (NORM=OP RIGHT) 'AND)
  ;; (OR L (AND R.L R.R)) ==> (AND (OR L R.L)(OR L R.R))
  (NORM=PUT=OP EXPRESSION 'AND)
          (NORM=PUT=LEFT EXPRESSION (NORM=MULTIPLY.TO.CNF (LIST 'OR LEFT (NORM=LEFT RIGHT))))
          (NORM=PUT=RIGHT EXPRESSION (NORM=MULTIPLY.TO.CNF (LIST 'OR (NORM=COPY LEFT) (NORM=RIGHT RIGHT)))) EXPRESSION)
        (T EXPRESSION)))))
  ...
  (let ((LEFT (NORM=LEFT EXPRESSION))
    ...
    (COND
      ((EQL (NORM=OP LEFT) 'OR)
       ...
       (NORM=PUT=LEFT EXPRESSION (NORM=MULTIPLY.TO.DNF (LIST 'AND (NORM=LEFT LEFT) RIGHT)))
       (NORM=PUT=RIGHT EXPRESSION (NORM=MULTIPLY.TO.DNF (LIST 'AND (NORM=RIGHT LEFT) (NORM=COPY RIGHT))))
       EXPRESSION)
      ((EQL (NORM=OP RIGHT) 'OR)
       ;; (AND L (OR R.L R.R)) ==> (OR (AND L R.L)(AND L R.R))
       (NORM=PUT=OP EXPRESSION 'OR)
       (NORM=PUT=LEFT EXPRESSION (NORM=MULTIPLY.TO.DNF (LIST 'AND LEFT (NORM=LEFT RIGHT))))
       (NORM=PUT=RIGHT EXPRESSION (NORM=MULTIPLY.TO.DNF (LIST 'AND (NORM=COPY LEFT) (NORM=RIGHT RIGHT))))
       EXPRESSION)
      (T EXPRESSION))))
  ...
  (let ((LEFT (NORM=LEFT EXPRESSION))
    ...
    (COND
      ((EQL (NORM=OP LEFT) 'OR)
       ;; (AND (OR L.L L.R) R) ==> (OR (AND L.L R)(AND L.R R))
       (NORM=PUT=OP EXPRESSION 'OR)
       (NORM=PUT=LEFT EXPRESSION (LIST 'AND! (NORM=LEFT LEFT) RIGHT))
       (NORM=PUT=RIGHT EXPRESSION (LIST 'AND! (NORM=RIGHT LEFT) (NORM=COPY RIGHT)))
       EXPRESSION)
      ((EQL (NORM=OP RIGHT) 'OR)
       ;; (AND L (OR R.L R.R)) ==> (OR (AND L R.L)(AND L R.R))
       (NORM=PUT=OP EXPRESSION 'OR)
       (NORM=PUT=LEFT EXPRESSION (LIST 'AND! LEFT (NORM=LEFT RIGHT)))
       (NORM=PUT=RIGHT EXPRESSION (LIST 'AND! (NORM=COPY LEFT) (NORM=RIGHT RIGHT)))
       EXPRESSION)
      (T EXPRESSION))))
...
(DEFUN NORM=COPY (EXPRESSION)
  ...
  (CASE (NORM=OP EXPRESSION)
    ...
    (NOT (LIST 'NOT (NORM=COPY (NORM=LEFT EXPRESSION))))
    ((AND OR IMPL EQV AND! OR! IMPL! EQV!)
...
(DEFUN NORM=VARIABLE.OCCURS.IN (VARIABLE EXPRESSION)
  ...
  (CASE (NORM=OP EXPRESSION)
    ...
    ((+ -) (IN VARIABLE (NORM=RIGHT EXPRESSION)))
...
(DEFVAR NORM*MIN.EXPRESSION.LENGTH.FOR.FILE 1)

(DEFVAR NORM*MIN.EXPRESSION.SIZE.FOR.FILE 1000000)
...
(DEFUN NORM=NEW.TERM (DOMAIN.VARIABLES RANGE.VARIABLE)
  ...
  (COND ((NULL DOMAIN.VARIABLES)
	 ...
	 (DT-CONSTANT.CREATE (CONCATENATE 'STRING "C_" (PRINC-TO-STRING NORM*CONSTANT.COUNTER))
			     (DT-VARIABLE.SORT RANGE.VARIABLE) T))
	(T (SETQ NORM*FUNCTION.COUNTER (1+ NORM*FUNCTION.COUNTER))
	   (CONS
	     (DT-FUNCTION.CREATE (CONCATENATE 'STRING (PRINC-TO-STRING "F_") (PRINC-TO-STRING NORM*FUNCTION.COUNTER))
