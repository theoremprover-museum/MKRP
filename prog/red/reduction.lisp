...
(DEFUN RED-REDUCE.DEDUCED.GRAPH (REMOVE.LITERALS REMOVE.CLAUSES RECOLOUR.UNIFIERS REDUCE.CLAUSES REDUCE.LINKS)
  ...
  (MAPC #'(LAMBDA (CLAUSE) (WHEN (DS-CLAUSE.IS CLAUSE)
	...
	RED*RW_RULES)
  (prog1 (COND
	   ((RED=CTL_REDUCE 'DEDUCED NIL REMOVE.CLAUSES REMOVE.LITERALS NIL NIL
			    RECOLOUR.UNIFIERS NIL NIL
			    REDUCE.CLAUSES REDUCE.LINKS (RDS-RULES 'DEDUCED)
			    (RDS-RULES 'DEDUCED.SUPPRESSED)))
	   ((NULL (CG-CLAUSES ALL)) (LIST 'FAILURE 'GRAPH.COLLAPSED)))
	 #-:mkrp-basic (nar-narrow (set-difference REDUCE.CLAUSES (cg-clauses removed)))))
...
(DEFUN RED-REDUCE.INITIAL.GRAPH NIL
  ...
  (prog1 (COND ((NULL (CG-CLAUSES ALL))
    ...
    #-:mkrp-basic(nar-narrow (cg-clauses all))))
...
(DEFUN RED=CTL_REMOVE.LITERAL (CLAUSE LITNO SIBLINGLITNO REASON INFO)
  ...
  (ds-clause.do #'(lambda (litno) 
		    (cond ((ds-clause.lit.rewrite.rule clause litno)
			   (setq red*rw_rules.completion (delete clause red*rw_rules.completion
								 :key #'first)))
			  ((ds-clause.lit.is.unfailing clause litno)
			   (setq red*rw_rules.unfailing (delete clause red*rw_rules.unfailing
								:key #'first)))))
		clause)
  (let* ((max (ds-clause.lit.is.max clause litno))
    ...
    (when (and (opt-is.kz.completion) (> (ds-clause.nolit clause) 0))
      ...
      (mapc #'(lambda (link)
		(red=info_link.result.replace link (op-paramodulate link (first (ds-link.unifiers link))
								    nil t) t))
	    (ds-clause.all.links 'p clause))
      (mapc #'(lambda (link)
		(red=info_link.result.replace link
					      (op-paramodulate link (first (ds-link.unifiers link)) nil t) t))
	    (ds-clause.all.links 'piw clause))
      (mapc #'(lambda (link)
		(red=info_link.result.replace link (op-resolve link (first (ds-link.unifiers link)) nil t) t))
