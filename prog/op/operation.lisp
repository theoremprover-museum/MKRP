		 ...
		 (let* ((pname (OP=res_NEWNAME))
		   ...
		   (setq origins nil)
		    #-:mkrp-basic(if (nar-narrow.info link)
				   ...
				   (PR-OPERATION 'RESOLUTION LINK UNIFIER RESOLVENT ren.subst))
		    #+:mkrp-basic(PR-OPERATION 'RESOLUTION LINK UNIFIER RESOLVENT ren.subst)
  ...
  (let ((NEGPAR (DS-LINK.NEGPAR LINK))
	...
	(if list.result
	    ...
	    (let* ((pname (OP=PAR_NEWNAME))
	      ...
	      (setq origins nil)
	      (PR-OPERATION 'PARAMODULATION LINK UNIFIER PARAMODULANT ren.subst)
  ...
  (let* ((CLAUSE (DS-LINK.POSPAR LINK))
	...
	(if list.result
	    ...
	    (let* ((pname (OP=fac_NEWNAME pname))
	      ...
	      (PR-OPERATION 'FACTORIZATION LINK UNIFIER FACTOR ren.subst)
...
(DEFUN OP-create.instance (UNIFIER clause &optional list.result)
  ...
  (let* ((ORIGINS (LIST NIL)) 
	...
	(let* ((pname (OP=inst_NEWNAME pname))
	  ...
	  (setq origins nil)
	  (PR-OPERATION 'instantiate UNIFIER clause instance ren.subst)
...
(DEFUN OP-CREATE.UNIT.FACTORS (CLAUSE)
  ...
  (COND ((EQL 1 (DS-CLAUSE.NOLIT CLAUSE)) NIL)
	...
	(T (let ((LITNOS (OP=SI.COMPLETE.GROUPS CLAUSE)) UNIFIERS LITNO1)
	     (when LITNOS
		   (SETQ UNIFIERS (LIST NIL))
		   (EVERY #'(lambda (LITNOS)
		   ...
		   (MAPCan #'(lambda (UNIFIER)
			       (OP=FACTORIZE (CAR (DS-CLAUSE.LINKS 'SI CLAUSE 1)) UNIFIER NIL nil nil))
			   UNIFIERS))))))
