...
(when (find-package "MKRP")
  (rename-package "MKRP" "MKRP5"))

(push :mkrp-basic *features*)
;(setq *features* (delete :mkrp-basic *features*))
...
(declaim (declaration edited authors input effect value))
...
(defun mkrp=boot=load-fas ()
  (mapc (function (lambda (f) (load (mkrp=make.path.out.comp f))))
         mkrp*boot_files))
...
(defun mkrp=boot=compile-all-new ()
  ...
  (mapc (function (lambda (f)
                    ...
                    (load (mkrp=make.path.out.comp f))))
        mkrp*boot_files))
