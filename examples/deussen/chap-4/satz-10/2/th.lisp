...
(COND ((FMT-LOAD "18-NOV-1987 01:53"
  NIL
  (((ALL PHI:SYMB.OF.FUNCTION ALL U:SET MAPPING (PHI S U) IMPL
   (ALL TEINS,TZWEI:EL.OF.S SET.EQUAL
    (PREIMAGE (IND.EQU (PHI U) TEINS) PREIMAGE (IND.EQU (PHI U) TZWEI)) IMPL APPLY (PHI TEINS)
    = APPLY (PHI TZWEI)))
  (ALL 25
   (ALL 24
    (IMPL (+ 10 (25 12 24) NIL)
     (ALL 27
      (ALL 26
       (IMPL (+ 3 ((20 (14 25 24) 26) (20 (14 25 24) 27)) NIL)
        (+ 9 ((11 25 26) (11 25 27)) NIL)))))))
  QUANTIFICATION)))(COND (ST*LOAD.FLAG (PROGN (DT-RESET))

(PROG ((ADDRLIST
        '(29758 29763 29773 29768 29778 29783 29788 29802 29807 29812 29817 29827 29822 29841
          29856 29861 29875 29890 29905 29910 29915 29920 29925 29939 29954 29969 29984
          29999))
       (DATALIST
      ...
      (COND ((> MEM*SIZE 30000) (SETQ MEM*NEXT.RADR (+ 29753 INCREMENT)))
            (T (SETQ MEM*NEXT.RADR 29753)))
...
(PROGN (PROGN (SETF (GET 'EQU.RELATION 'DT*SORT.NUMBER) '7)
       ...
       (SETQ ST*SYMBOL.ADDRESSES
              '(NIL (ANY . T) (TRUE . 0) (FALSE . 1) (SET . T) (ELEMENT . T) (EL . 2)
                ...
                (X_22 . 22) (X_23 . 23) (X_25 . 25) (X_24 . 24) (X_26 . 26) (X_27 . 27))))))))