...
#+allegro-v4.1
(progn (setq *enable-package-locked-errors* nil)
...
(defun mkrp=make.path.out.comp (f)
  ...
  #+allegro-v4.1(make-pathname :type "fasl" :defaults 
			       (make-pathname  :defaults f))
  #-(or lcl4.0 allegro-v4.1)(make-pathname :type #+kcl"o"#-kcl"lbin" :defaults f))  ; Our KCL version doesn't accept other types than "o"
...
(defun mkrp-dumper (&optional (filename "mkrp"))
  (mkrp::mkrp-init)
  #+allegro-v4.1
    (progn
    ...
    (excl:dumplisp :name filename :checkpoint nil
		   :read-init-file nil
		   :restart-function #'mkrp-start))
  #+lucid(user::disksave filename :restart-function #'mkrp-start :full-gc t)			 
  #+kcl(system:save-system filename)
  #+coral(user::disksave "Scuzzy C:mkrp:mkrp" :restart-function #'mkrp-start :full-gc t)
  #+poplog(lisp::savelisp filename)
  #-(or kcl lucid coral allegro-v4.1)(format t "System dependent, implement yourself."))


; Repair for Kyoto and poplog:

#+(or poplog kcl allegro-v4.1)(defun ignore (&rest nix) nil)

#+(or poplog kcl allegro-v4.1)(compile 'ignore)
